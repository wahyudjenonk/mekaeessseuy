...
class Lib {
	...
	function kirimemail($type="", $email="", $p1="", $p2="", $p3=""){
		...
		$subject = "";
		switch($type){
			case "email_register_sales":
				$ci->nsmarty->assign('email', $email);
				$ci->nsmarty->assign('password', $p1);
				$ci->nsmarty->assign('kodemarketing', $p2);
				$ci->nsmarty->assign('namalengkap', $p3);
				$html = $ci->nsmarty->fetch('backend/modul/setting/email_registrasi_sales.html');
				$subject = "REGISTRASI SALES & MARKETING MKS-Store";
			break;
